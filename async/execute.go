...
func ExecAsync(db *sql.DB, query string, params map[string]interface{}) (AsyncResult, error) {
...
func ExecAsyncContext(ctx context.Context, db *sql.DB, query string, params map[string]interface{}) (AsyncResult, error) {
	...
	if asyncRes, ok := res.(AsyncResult); ok {
		return asyncRes, nil
	}
	return AsyncResult{}, fmt.Errorf("failed to execute async query, unexpected result type %T", res)
}
